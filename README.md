...
`flock` is a single DuckDB extension. When loaded, your DuckDB instance
...
LOAD '/path/to/build/release/extension/flock/flock.duckdb_extension';
...
│           uri               │         url           │             token               │
├─────────────────────────────┼───────────────────────┼─────────────────────────────────┤
│ flock:127.0.0.1:9494        │ http://127.0.0.1:9494 │ a1b2c3d4e5f6...                 │
└─────────────────────────────┴───────────────────────┴─────────────────────────────────┘
```

Open `http://127.0.0.1:9494/`. flock asks you to **paste the token
...
`flock` runs **one HTTP server on one port** and serves three protocols
...
flock server as a first-class catalog. The wire protocol on `/quack`
is exactly upstream Quack — flock is a strict superset.
...
CREATE SECRET (
  TYPE quack,
  TOKEN 'a1b2c3d4...',                        -- the token from flock_serve
...
> Stock DuckDB-Quack clients use the `quack:` URI scheme. flock servers
> also accept `flock:` for tooling that's flock-aware; the two resolve
> to the same handler.
...
curl -X POST http://localhost:9494/sql \
  -H "Authorization: Bearer $FLOCK_TOKEN" \
...
SID=$(curl -sf -X POST http://localhost:9494/sql/sessions/new \
        -H "Authorization: Bearer $FLOCK_TOKEN" | jq -r .sessionId)
...
# 3. Optional cleanup (also auto-expires after flock_session_ttl_s)
...
DuckDB-Wasm builds with the `quack` extension can `ATTACH` a flock
...
`flock` is a single DuckDB extension forked from two upstream projects:
...
flock adds the `/sql` JSON/NDJSON endpoint, an HMAC-signed auth-cookie
...
flock login wrapper at `GET /`).

## Security

flock is **trusted-network software** by default. The auth token grants
...
CREATE TABLE flock_tokens (token VARCHAR, user_name VARCHAR);
INSERT INTO flock_tokens VALUES
  ('alice-key-...', 'alice'),
  ('bob-key-...',   'bob');

CREATE MACRO check_token(sid, client_token, server_token) AS (
  EXISTS (SELECT 1 FROM flock_tokens WHERE token = client_token)
);
SET GLOBAL flock_authentication_function = 'check_token';

-- Read-only authorization (admin endpoints use synthetic SQL of shape
-- '__FLOCK_ADMIN__:<resource>:<action>' — e.g. '__FLOCK_ADMIN__:checkpoint:create',
-- '__FLOCK_ADMIN__:sessions:list' — so policies can branch on prefix or pair)
...
| Clone for staging | `zfs clone …@last-good …-staging && incus launch flock-image …` |
| Restore | `zfs rollback …@some-snapshot && incus restart …` |
| Move host | `zfs send | ssh other zfs receive && incus copy …` |
| Upgrade flock | drop new `.duckdb_extension` into image, `incus restart` |
...
|---|---|---|
| `flock_bind` | `127.0.0.1` | Bind address |
| `flock_port` | `9494` | Listen port |
| `flock_token` | (auto-generated) | Auth token |
| `flock_authentication_function` | `flock_check_token` | SQL function name for auth callback |
| `flock_authorization_function` | `flock_nop_authorization` | SQL function name for authz callback |
| `flock_cookie_signing_key` | (auto, per-process) | HMAC key for `flock_session` cookies |
| `flock_max_sessions` | `1024` | Max concurrent DB sessions |
...
- Built-in TLS
- Multi-tenant proxy in front of N flocks (lives in a separate ops project)
- A flock client extension that registers `flock:` natively on the client side

## Requirements

- **DuckDB** v1.5.2 (each flock release pins to a specific DuckDB version)
