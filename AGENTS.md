...
| **PR-7+** | Hardening, full CI matrix (`linux_amd64`/`linux_arm64`, `osx_amd64`/`osx_arm64`, `windows_amd64`/`windows_amd64_mingw`, `wasm_mvp`/`wasm_eh`/`wasm_threads` — 9 build targets via `reduced_ci_mode: 'disabled'`), `harbor_query_timeout_s` runtime enforcement, default-deny on unknown `Authorization:` schemes, login-page CSP+nonce, full nested-type Mode B parser, per-DuckDB-type `/sql` round-trip + byte-level Quack/UI golden fixtures, community-extensions distribution. Split into focused PRs: PR-7a (CI matrix flip, in flight), PR-7b/c/... (one item per PR). | |
...
- **Never break Quack wire compatibility.** A vanilla DuckDB client with
  ...
  `test/sql/harbor.test` (introduced PR-1.5) is the current regression
...
| `src/harbor_extension.cpp` | Entry point: registers settings, scalar functions, `harbor_serve`/`harbor_stop`/`harbor_wait` table macros | Yes |
| `src/harbor_http_server.{cpp,hpp}` | `HarborHttpServer` — owns the cpp-httplib `Server`, holds shared state | Yes |
| `src/harbor_session.{cpp,hpp}` | `SessionManager` — per-session DuckDB `Connection` pool with mutex | Yes |
| `src/harbor_auth.{cpp,hpp}` | `AuthManager` — token + hook resolution, HMAC cookie sign/verify (PR-4), CORS allow-list parsing (PR-4) | Yes |
| `src/harbor_crypto.{cpp,hpp}` | OpenSSL libcrypto wrapper: SHA-256, HMAC-SHA256, RAND_bytes, base64url, constant-time-equal (PR-4) | Yes |
| `src/auth_handlers.{cpp,hpp}` | `AuthHandlers` — `POST /auth/login`, `POST /auth/logout`, `OPTIONS /auth/*`, `OPTIONS /quack` (PR-4) | Yes |
| `src/harbor_log.{cpp,hpp}` | `'Harbor'` and `'HTTP'` log type registration | Yes |
| `src/harbor_wait.{cpp,hpp}` | Blocking `harbor_wait()` table function — keeps the DuckDB process alive in container/daemon mode | Yes |
...
| `src/quack/quack_scan.{cpp,hpp}`, `src/quack/storage/*` | Client side (`ATTACH 'quack:host'` for stock clients; `ATTACH 'harbor:host'` when using Harbor's compatibility storage key), unchanged from upstream Quack except for registration glue | Carefully |
...
| `src/ui/ui_assets.{cpp,hpp}` | `GET /.*` from bundled assets (when `harbor_ui_assets='bundled'`) | Yes |
...
| `scripts/golden-quack-roundtrip.sh` | Regression: stock Quack client ↔ harbor | Yes |
| `scripts/golden-ui-roundtrip.sh` | Regression: official UI ↔ harbor | Yes |
...
| `test/integration/` | End-to-end: spin a harbor, hit it, assert | Yes |
...
1. Add to `src/harbor_extension.cpp` in the settings block.
