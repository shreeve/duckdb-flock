...
#   - NDJSON chunk mode
#   - One-shot JSON mode
#   - Request validation: missing sql, multi-statement, __FLOCK_ADMIN__, oversize body
...
PORT="${FLOCK_SQL_TEST_PORT:-19506}"
...
cleanup() {
    ...
    rm -f "${SERVER_LOG}" "${COOKIE_JAR}" /tmp/flock-sql-*.json /tmp/flock-sql-*.ndjson /tmp/flock-sql-big-body.json
}
...
SET GLOBAL flock_max_request_body_bytes=1024;
...
curl -sfN -X POST "http://127.0.0.1:${PORT}/sql" \
    ...
    > /tmp/flock-sql-row.ndjson
grep -q '"type":"schema"' /tmp/flock-sql-row.ndjson || fail "row mode missing schema"
grep -q '"duckdbType":"INTEGER"' /tmp/flock-sql-row.ndjson || fail "row mode missing INTEGER schema"
grep -q '"values":\[42,"hello"\]' /tmp/flock-sql-row.ndjson || fail "row mode missing row values"
grep -q '"type":"end"' /tmp/flock-sql-row.ndjson || fail "row mode missing end"
...
curl -sfN -X POST "http://127.0.0.1:${PORT}/sql" \
    ...
    > /tmp/flock-sql-chunk.ndjson
grep -q '"type":"chunk"' /tmp/flock-sql-chunk.ndjson || fail "chunk mode missing chunk line"
grep -q '"rows":\[\["0"\],\["1"\],\["2"\]\]' /tmp/flock-sql-chunk.ndjson || fail "chunk mode rows mismatch"
...
curl -sf -X POST "http://127.0.0.1:${PORT}/sql" \
    ...
    > /tmp/flock-sql-oneshot.json
grep -q '"ok":true' /tmp/flock-sql-oneshot.json || fail "one-shot missing ok"
grep -q '"kind":"select"' /tmp/flock-sql-oneshot.json || fail "one-shot missing kind"
grep -q '"data":\[\[1,2\]\]' /tmp/flock-sql-oneshot.json || fail "one-shot data mismatch"
pass "POST /sql one-shot JSON"

# ---- Validation failures ----
code="$(curl -s -o /tmp/flock-sql-missing.json -w '%{http_code}' \
    -X POST "http://127.0.0.1:${PORT}/sql" \
    -H "Authorization: Bearer ${TOKEN}" \
    -H 'Content-Type: application/json' \
    -d '{}')"
[[ "${code}" == "400" ]] || fail "missing sql expected 400, got ${code}"
grep -q '"errorCode":"BAD_REQUEST"' /tmp/flock-sql-missing.json || fail "missing sql error code"
pass "missing sql rejected"

code="$(curl -s -o /tmp/flock-sql-multi.json -w '%{http_code}' \
    -X POST "http://127.0.0.1:${PORT}/sql" \
    -H "Authorization: Bearer ${TOKEN}" \
    -H 'Content-Type: application/json' \
    -d '{"sql":"SELECT 1; SELECT 2;"}')"
[[ "${code}" == "400" ]] || fail "multi-statement expected 400, got ${code}"
grep -q '"errorCode":"BAD_REQUEST"' /tmp/flock-sql-multi.json || fail "multi-statement error code"
pass "multi-statement rejected"

code="$(curl -s -o /tmp/flock-sql-admin.json -w '%{http_code}' \
    -X POST "http://127.0.0.1:${PORT}/sql" \
    -H "Authorization: Bearer ${TOKEN}" \
    -H 'Content-Type: application/json' \
    -d '{"sql":"__FLOCK_ADMIN__:checkpoint:create"}')"
[[ "${code}" == "400" ]] || fail "__FLOCK_ADMIN__ expected 400, got ${code}"
pass "__FLOCK_ADMIN__ reserved prefix rejected"

python3 - <<'PY' >/tmp/flock-sql-big-body.json
print('{"sql":"' + ('x' * 2000) + '"}')
PY
code="$(curl -s -o /tmp/flock-sql-big-response.json -w '%{http_code}' \
    -X POST "http://127.0.0.1:${PORT}/sql" \
    -H "Authorization: Bearer ${TOKEN}" \
    -H 'Content-Type: application/json' \
    --data-binary @/tmp/flock-sql-big-body.json)"
[[ "${code}" == "413" ]] || fail "oversized body expected 413, got ${code}"
pass "oversized body rejected"

# ---- Auth failures and cookie auth ----
code="$(curl -s -o /tmp/flock-sql-auth.json -w '%{http_code}' \
    -X POST "http://127.0.0.1:${PORT}/sql" \
...
LOGIN="$(curl -s -i -X POST "http://127.0.0.1:${PORT}/auth/login" \
    -H 'Content-Type: application/json' \
    -d "{\"token\":\"${TOKEN}\"}")"
COOKIE="$(echo "${LOGIN}" | grep -i '^Set-Cookie: flock_session=' | sed -E 's/^[Ss]et-[Cc]ookie: flock_session=([^;]+).*$/\1/' | tr -d '\r')"
[[ -n "${COOKIE}" ]] || fail "failed to obtain flock_session cookie"
curl -sfN -X POST "http://127.0.0.1:${PORT}/sql" \
    -b "flock_session=${COOKIE}" \
    -H 'Origin: https://app.example.com' \
    -H 'Content-Type: application/json' \
    -d '{"sql":"SELECT 7 AS cookie_ok"}' \
    > /tmp/flock-sql-cookie.ndjson
grep -q '"values":\[7\]' /tmp/flock-sql-cookie.ndjson || fail "cookie-auth /sql did not return row"
...
curl -sfN -X POST "http://127.0.0.1:${PORT}/sql" \
    ...
    > /tmp/flock-sql-param.ndjson
grep -q '"values":\[42\]' /tmp/flock-sql-param.ndjson || fail "implicit param failed"
...
curl -sfN -X POST "http://127.0.0.1:${PORT}/sql" \
    ...
    > /tmp/flock-sql-typed-param.ndjson
grep -q '"duckdbType":"DECIMAL(18,4)"' /tmp/flock-sql-typed-param.ndjson || fail "typed decimal schema missing"
grep -q '"values":\["123.4567",null\]' /tmp/flock-sql-typed-param.ndjson || fail "typed param row mismatch"
...
curl -sfN -X POST "http://127.0.0.1:${PORT}/sql" \
    ...
    > /tmp/flock-sql-types.ndjson
grep -q '"9223372036854775807"' /tmp/flock-sql-types.ndjson || fail "BIGINT should be string"
grep -q '"12345.6789"' /tmp/flock-sql-types.ndjson || fail "DECIMAL should be string"
grep -q '"months":12,"days":2,"micros":"3000000"' /tmp/flock-sql-types.ndjson || fail "INTERVAL object mismatch"
grep -q '"aGVsbG8gd29ybGQ="' /tmp/flock-sql-types.ndjson || fail "BLOB base64 mismatch"
grep -q '"{\\"a\\":1}"' /tmp/flock-sql-types.ndjson || fail "JSON column should be JSON-text string"
...
curl -sfN -X POST "http://127.0.0.1:${PORT}/sql" -H "Authorization: Bearer ${TOKEN}" \
    ...
    > /tmp/flock-sql-session.ndjson
grep -q '"values":\["3"\]' /tmp/flock-sql-session.ndjson || fail "session transaction state not visible"
...
code="$(curl -s -o /tmp/flock-sql-session-gone.json -w '%{http_code}' \
...
curl -sf -X POST "http://127.0.0.1:${PORT}/auth/logout?destroy_sessions=true" \
    -b "flock_session=${COOKIE}" -d '' >/tmp/flock-sql-logout.json
code="$(curl -s -o /tmp/flock-sql-session-destroyed.json -w '%{http_code}' \
    -X POST "http://127.0.0.1:${PORT}/sql" -H "Authorization: Bearer ${TOKEN}" \
